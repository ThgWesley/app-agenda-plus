...
function applyProfile() {
    ...
    // Preenche os campos do perfil com os valores salvos
    ...
    if(colorVal) colorVal.textContent = profile.color || '#6200ea';

    // Foto do perfil
    if(profile.photo) {
        const img = document.getElementById('profile-preview');
        const emoji = document.getElementById('profile-emoji');
        const mini = document.getElementById('header-profile-pic');
        if(img) { img.src = profile.photo; img.style.display = 'block'; }
        if(emoji) emoji.style.display = 'none';
        if(mini) mini.innerHTML = `<img src="${profile.photo}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
    }

    if(typeof Finance !== 'undefined') Finance.adjustCardTextColor();
}
...
if(btnSaveProfile) {
    btnSaveProfile.addEventListener('click', () => {
        const nameInput  = document.getElementById('company-name-input');
        const colorInput = document.getElementById('app-color-input');
        const profile    = Storage.get('profile') || { name: 'Agenda Plus', color: '#6200ea', photo: null };

        profile.name  = nameInput ? nameInput.value : profile.name;
        profile.color = colorInput ? colorInput.value : profile.color;

        // Foto do perfil
        const uploadInput = document.getElementById('profile-upload');
        const previewImg  = document.getElementById('profile-preview');
        if(previewImg && previewImg.src && previewImg.src.includes('data:image')) {
            profile.photo = previewImg.src;
        }
        ...
        setTimeout(() => { btnSaveProfile.innerText = 'Salvar Alterações'; }, 2000);
    });
}

// Atualiza preview da cor em tempo real
const colorInput = document.getElementById('app-color-input');
const colorValDisplay = document.getElementById('color-val-display');
if(colorInput) {
    colorInput.addEventListener('input', () => {
        if(colorValDisplay) colorValDisplay.textContent = colorInput.value;
        // Preview instantâneo
        document.documentElement.style.setProperty('--primary', colorInput.value);
        document.documentElement.style.setProperty('--primary-light', colorInput.value + '22');
    });
}
...
function loadNotifSettings() {
    const s = Storage.get('notifSettings') || { dayBefore: false, dayOf: false };
    const cb1 = document.getElementById('notif-day-before');
    const cb2 = document.getElementById('notif-day-of');
    if(cb1) cb1.checked = s.dayBefore;
    if(cb2) cb2.checked = s.dayOf;
}

function scheduleNotifications() {
    const clients = Storage.get('clients') || [];
    const s = Storage.get('notifSettings') || {};
    if(!s.dayBefore && !s.dayOf) return;

    clients.forEach(c => {
        const eventDate = new Date(c.date + 'T08:00:00');
        const now = new Date();
        ...
        if(s.dayOf) {
            ...
            if(diff > 0) {
                setTimeout(() => {
                    new Notification('📅 Evento Hoje!', {
                        body: `${c.clientName} — ${c.kitName || c.category}`,
        ...
        if(s.dayBefore) {
            ...
            const diff = dayBefore - now;
            if(diff > 0) {
                setTimeout(() => {
                    new Notification('🔔 Evento Amanhã!', {
                        body: `${c.clientName} — ${c.kitName || c.category}`,
                        icon: 'icons/icon-192.png'
                    });
                }, diff);
...
if(btnSaveNotif) {
    btnSaveNotif.addEventListener('click', async () => {
        ...
        const cb1 = document.getElementById('notif-day-before');
        const cb2 = document.getElementById('notif-day-of');
        const dayBefore = cb1 ? cb1.checked : false;
        const dayOf     = cb2 ? cb2.checked : false;

        if(!('Notification' in window)) {
            if(statusEl) { statusEl.textContent = '⚠️ Seu navegador não suporta notificações.'; statusEl.style.display = 'block'; }
            return;
        }

        const perm = await Notification.requestPermission();
        if(perm !== 'granted') {
            if(statusEl) { statusEl.textContent = '❌ Permissão negada. Ative nas configurações do navegador.'; statusEl.style.display = 'block'; }
            return;
        }

        Storage.set('notifSettings', { dayBefore, dayOf });
        scheduleNotifications();

        if(statusEl) {
            statusEl.textContent = '✅ Notificações salvas!';
            statusEl.style.color = 'var(--green)';
            statusEl.style.display = 'block';
            setTimeout(() => { statusEl.style.display = 'none'; }, 3000);
        }
        btnSaveNotif.innerText = '✅ Salvo!';
        setTimeout(() => { btnSaveNotif.innerText = 'Salvar Notificações'; }, 2000);
    });
}
...
if(btnInstallPWA) {
    // Esconde inicialmente; aparece só quando o browser permitir instalar
    btnInstallPWA.style.display = 'none';
    btnInstallPWA.addEventListener('click', async () => {
        if(!deferredInstallPrompt) {
            // iOS Safari: instrução manual
            alert('No iPhone/iPad: toque em Compartilhar (□↑) e depois "Adicionar à Tela de Início".');
            return;
        }
...
if(btnExportIcs) {
    btnExportIcs.addEventListener('click', () => {
        ...
        const events = clients.map(c => {
            const uid = `${c.id}@agendaplus`;
            const summary = `${c.clientName} — ${c.kitName || c.category || 'Evento'}`;
            const desc = `Kit: ${c.kitName || '—'} | Categoria: ${c.category || '—'} | Valor: R$ ${(parseFloat(c.value)||0).toFixed(2).replace('.',',')} | Status: ${c.status}`;
            return [
                'BEGIN:VEVENT',
                `UID:${uid}`,
                `DTSTART:${toICSDate(c.date)}`,
                `DTEND:${toICSDateEnd(c.date)}`,
                `SUMMARY:${summary}`,
                `DESCRIPTION:${desc}`,
                'END:VEVENT'
            ].join('\r\n');
        ...
        const ics = [
            'BEGIN:VCALENDAR',
            'VERSION:2.0',
            `PRODID:-//${profile.name}//AgendaPlus//PT`,
            'CALSCALE:GREGORIAN',
            'METHOD:PUBLISH',
            events,
            'END:VCALENDAR'
        ...
        link.download = `AgendaPlus_${new Date().toLocaleDateString('pt-BR').replace(/\//g,'-')}.ics`;
        link.click();
    });
}
